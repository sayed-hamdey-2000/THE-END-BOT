...
import cfonts from 'cfonts'; // إضافة مكتبة cfonts
...
export default async function(m, conn = { user: {} }) {
  ...
  // استخدم cfonts لعرض عنوان أو شعار بالخط الكبير
  cfonts.say('معلومات الرسالة', {
    font: 'block',
    align: 'left',
    colors: ['cyan', 'yellow'],
...
▣────────────···
`.trim(),
    me + ' ~' + conn.user.name + `${conn.user.jid == global.conn.user.jid ? '' : ' (بوت فرعي)'}`,
    ...
    m.chat + (chat ? ' ~' + chat : ''),
    m.mtype ? m.mtype.replace(/message$/i, '').replace('audio', m.msg.ptt ? 'PTT' : 'صوت').replace(/^./, (v) => v.toUpperCase()) : '',
  );
  ...
  else if (/ContactsArray/i.test(m.mtype)) console.log(`👨‍👩‍👧‍👦 ${'جهات الاتصال' || ''}`);
  else if (/contact/i.test(m.mtype)) console.log(`👨 ${m.msg.displayName || 'جهة اتصال'}`);
  else if (/audio/i.test(m.mtype)) {
    const duration = m.msg.seconds;
    console.log(`${m.msg.ptt ? '🎤ㅤ(PTT ' : '🎵ㅤ('}صوت) ${Math.floor(duration / 60).toString().padStart(2, 0)}:${(duration % 60).toString().padStart(2, 0)}`);
  }
  else if (/image/i.test(m.mtype)) console.log(`🖼️ ${'صورة'}`);
  else if (/video/i.test(m.mtype)) console.log(`🎥 ${'فيديو'}`);
}

const file = global.__filename(import.meta.url);
watchFile(file, () => {
  console.log(chalk.redBright('تحديث \'lib/print.js\''));
});
